...
internal_setent (FILE **stream)
{
  ...
  if (*stream == NULL)
    {
      *stream = fopen ("/usr/etc/aliases", "rce");
...
enum nss_status
_nss_usrfiles_setaliasent (void)
{
...
/* Thread-safe, exported version of that.  */
enum nss_status
_nss_usrfiles_endaliasent (void)
{
  __libc_lock_lock (lock);
...
enum nss_status
_nss_usrfiles_getaliasent_r (struct aliasent *result, char *buffer, size_t buflen,
			  int *errnop)
{
...
enum nss_status
_nss_usrfiles_getaliasbyname_r (const char *name, struct aliasent *result,
			     char *buffer, size_t buflen, int *errnop)
{
  /* Return next entry in host file.  */
  enum nss_status status = NSS_STATUS_SUCCESS;
