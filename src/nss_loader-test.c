...
main (void)
{
  ...
  nss_handle = dlopen (".libs/libnss_usrfiles.so.2", RTLD_NOW);
  if (!nss_handle)
    {
      fprintf (stderr, "dlopen(\".libs/libnss_usrfiles.so.2\") failed:\n%s\n",
	       dlerror ());
      return 1;
    }

  nss_getpwnam_r = dlsym (nss_handle, "_nss_usrfiles_getpwnam_r");
  if ((error = dlerror ()) != NULL)
    {
      fprintf (stderr, "dlsym(\"_nss_usrfiles_getpwnam_r\") failed: %s\n",
	       error);
      dlclose (nss_handle);
      return 1;
    }
