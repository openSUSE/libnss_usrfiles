...
#define DATAFILE	"/usr/etc/" DATABASE
...
enum nss_status
CONCAT(_nss_usrfiles_set,ENTNAME) (int stayopen)
{
...
/* Thread-safe, exported version of that.  */
enum nss_status
CONCAT(_nss_usrfiles_end,ENTNAME) (void)
{
  __libc_lock_lock (lock);
...
enum nss_status
CONCAT(_nss_usrfiles_get,ENTNAME_r) (struct STRUCTURE *result, char *buffer,
...
enum nss_status								      \
_nss_usrfiles_get##name##_r (proto,					      \
